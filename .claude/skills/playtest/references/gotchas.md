...
- **`wt-action-spell-<slug>-*` and `wt-spell-<slug>-*` are DIFFERENT surfaces.** The Combat tab's action rows (`WtActionRow`) emit `wt-action-spell-<slug>-roll`; the Spells tab (`WtSpellsTab`) emits `wt-spell-<slug>-cast`. Both panes are mounted at once, so a probe anchored on the wrong prefix reads the *inactive* pane and its correct `inert`/hidden state looks like a broken control. **This produced a false issue (#2171) on 2026-08-04.** Before concluding a control is broken, print the element's `data-testid` and confirm which pane owns it.
...
- **`visibility: hidden` reads as "not visible" to Playwright even with a real rect.** When a click times out on an element that clearly exists, check `getComputedStyle(el).visibility` before assuming a z-order or overlay problem — a hidden element and an overlay eating clicks (#2173, the maps drawer) need different fixes. But confirm which pane owns the element FIRST: the "hidden cast buttons" that seeded this entry were the inactive Spells pane correctly hidden (#2171, retracted — see the wrong-prefix trap above).
