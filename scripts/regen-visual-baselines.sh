...
# #2211 — VISUAL_ALL_THEMES was set here so a regen swept all 13 themes rather
# than the 4 the per-PR gate compared, otherwise a regen left 9 themes' baselines
# stale. Those 9 themes and their baselines are deleted: the spec's THEMES list is
# the whole matrix now, so regen and the gate cover the same set by construction
# and there is nothing left for this variable to widen.
