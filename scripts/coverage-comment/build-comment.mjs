...
export function renderSplitTable(files) {
  let unitCov = 0, unitTotal = 0, intCov = 0, intTotal = 0, combCov = 0, combTotal = 0;
...
export function renderSnippet(sourceLines, coveredLines, uncoveredLines, gutterMap, coverageEntry = null) {
...
export function render(files, opts, readSource) {
  ...
  if (dualMode && totalChanged > 0) {
    ...
    lines.push("");
    lines.push("<sub>Gutter markers in snippets: `U` = unit only · `I` = integration only · `UI` = both</sub>");
  }
  ...
  if (withUncovered.length > 0) {
    ...
    for (const f of withUncovered) {
      ...
      const snip = source
        ? renderSnippet(source.split("\n"), f.covered, f.uncovered, f.gutter ?? null, f.coverageEntry ?? null)
        : null;
      if (snip) {
        lines.push("");
        const legendGutter = dualMode ? " · <code>U</code>=unit <code>I</code>=int <code>UI</code>=both" : "";
        lines.push(`<sub>bright bar = new line covered/uncovered · muted bar = existing line · no bar = non-executable${legendGutter}</sub>`);
        lines.push("");
        lines.push(snip);
      }
      lines.push("");
  ...
  lines.push(footer);
  lines.push("<sub>SQL, YAML, shell, docs, config, and test files are excluded — only JS/TS with new executable lines appear above.</sub>");
  return lines.join("\n");
}
...
function main() {
  ...
  const COVERAGE_FILES_UNIT = process.env.COVERAGE_FILES_UNIT
    ? process.env.COVERAGE_FILES_UNIT.split(",").map((s) => s.trim()).filter(Boolean)
    : null;
  const COVERAGE_FILES_INT = process.env.COVERAGE_FILES_INT
    ? process.env.COVERAGE_FILES_INT.split(",").map((s) => s.trim()).filter(Boolean)
    : null;
