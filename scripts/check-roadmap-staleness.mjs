...
function parseArgs(argv) {
  const args = { weeks: 5, file: null, strict: false };
...
function main() {
  ...
  if (!result.found) {
    console.warn(
      `[roadmap] WARNING: no "Last reviewed:" line in ${file}. Add one (YYYY-MM-DD) so the heartbeat can run.`,
    );
  ...
  if (result.stale) {
    console.warn(
      `[roadmap] WARNING: ROADMAP last reviewed ${result.lastReviewed} (${result.daysOld} days ago, ` +
        `threshold ${args.weeks}w). Re-read it against open milestones + recent CHANGELOG, then bump "Last reviewed:".`,
    );
    process.exit(args.strict ? 1 : 0);
  }

  console.log(
    `[roadmap] OK — last reviewed ${result.lastReviewed} (${result.daysOld} days ago, under the ${args.weeks}w threshold).`,
  );
}
